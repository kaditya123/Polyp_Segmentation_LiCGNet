...
from dataset.Kvasir_CVCclinic import KvasirSEG_ClinicDB_dataset
...
def val(args, val_loader, model, criterion):
    ...
    for i, (input, label, size, name) in enumerate(val_loader):
        ...
        input_var = Variable(input, volatile=True).cuda()    ## Change cuda->CPU if device changed from GPU to other
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    return average_epoch_loss_train, per_class_iu, meanIoU, lr, precision, recall, f1_score, accuracy
...
def train_model(args):
    ...
    model = CGNet.CGNET(classes= args.classes, M= M, N= N)     # change the model name based on the selected model
    # model = RCGNet.Residual_Context_Guided_Network(classes= args.classes, M= M, N= N)

    network_type="CGNet"       # Change the name based on the selected model
    ...
    args.savedir = ( args.savedir + args.dataset + '/'+ network_type +"_M"+ str(M) + 'N' +str(N) + '_bs' 
    ...
    train_transform= transforms.Compose([
        transforms.ToTensor()])
    trainLoader = data.DataLoader( KvasirSEG_ClinicDB_dataset( args.data_dir, args.train_data_list, crop_size = input_size, 
                                            scale = args.random_scale, mirror = args.random_mirror, mean = datas['mean'] ),
                                            batch_size = args.batch_size, shuffle = True, num_workers = args.num_workers, 
                                            pin_memory = True, drop_last = True )
    
    valLoader = data.DataLoader( KvasirSEG_ClinicDB_Val_dataset( args.data_dir, args.val_data_list,f_scale = 1,  
                                                               mean = datas['mean']),
    ...
    if os.path.isfile(logFileLoc):
        ...
        logger.write("\n%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t" % ('Epoch', 'Loss(Tr)', 'mIOU (tr)', 'mIOU (val)', 'Precision(Tr)', 'Precision(val)', 'Recall(Tr)', 'Recall(val)', 'F1-score(Tr)', 'F1-score(val)', 'Accuracy(Tr)', 'Accuracy(val)', 'Learn-Rate'))
    logger.flush()
    ...
    for epoch in range(start_epoch, args.max_epochs):
        #training
        lossTr, per_class_iu_tr, mIOU_tr, lr, precision_tr, recall_tr, f1_tr, accuracy_tr = train(args, trainLoader, model, criteria, optimizer, epoch)
        
        #validation
        if epoch % 25 ==0:
            mIOU_val, per_class_iu, precision_val, recall_val, f1_val, accuracy_val = val(args, valLoader, model, criteria)
            logger.write("\n%d\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.7f" % (epoch, lossTr, mIOU_tr, mIOU_val, precision_tr, precision_val, recall_tr, recall_val, f1_tr, f1_val, accuracy_tr, accuracy_val, lr))
            logger.flush()
            print("Epoch: " + str(epoch) + ' Details')
            print("\nEpoch No.: %d\t  Train Loss = %.4f\t mIOU(tr) = %.4f\t Precision = %.4f\t Recall = %.4f\t F1-score = %.4f\t Accuracy = %.4f\t lr= %.6f" % (epoch,
                   lossTr, mIOU_tr, precision_tr, recall_tr, f1_tr, accuracy_tr,lr))
            print("\nVALIDATION SCORES:  mIOU(val) = %.4f\t Precision = %.4f\t Recall = %.4f\t F1-score = %.4f\t Accuracy = %.4f\t" % (mIOU_val, precision_val, recall_val, f1_val, accuracy_val))
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type = int, default = 8, help = "the batch size is set to 8 for 2 GPUs")
    parser.add_argument('--lr', type = float, default = 1e-3, help = "initial learning rate")
    parser.add_argument('--savedir', default = "./checkpoint/", help = "directory to save the model snapshot")
    parser.add_argument('--resume', type = str, default = "./checkpoint/KvasirSeg_CVCClinic/CGNet_M3N21bs8gpu1_ontrain/model_1.pth", 
                    help = "use this file to load last checkpoint for continuing training")  
    parser.add_argument('--classes', type = int, default = 2, 
                    help = "the number of classes in the dataset. 2 classes for all the considered datasets respectively")
