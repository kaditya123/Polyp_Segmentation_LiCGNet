...
__all__ = ["CGNET"]   # Only export CGNET class module when imported
...
class ConvBNPReLU(nn.Module):
    def __init__(self, nIn, nOut, k_size, stride = 1):
        """
        args:
        ...
        self.conv = nn.Conv2d(nIn, nOut, kernel_size = (k_size, k_size), stride = stride, padding = (padding, padding), bias = False)
        self.bn = nn.BatchNorm2d(nOut, eps = 1e-03)
        self.act = nn.PReLU(nOut)
...
class CGNET(nn.Module):
    def __init__(self, classes = 2, M = 3, N = 21, dropout_flag = False):
        """
            classes: Number of classes in the dataset. Default is 2 for polyp segmentation. '0' for background and '1' for polyp.
            M: Number of CG-Blocks in stage-2. Default is 3.
            N: Number of CG-Blocks in stage-3. Default is 21.
        """
        super().__init__()   # Initialize the parent class nn.Module

        ####################################### Encoder Stage 1 ##########################################
        # The first stage is a simple encoder with 3 convolutional layers, each followed by Batch Normalization 
        # and Parametric-ReLU activation.
        # The input image is downsampled by a factor of 2 in the first layer.

        self.encoder1_0 = ConvBNPReLU(nIn = 3, nOut = 32, k_size = 3, stride = 2) # input: (H, W, 3) --> output: (H/2, W/2, 32)
        self.encoder1_1 = ConvBNPReLU(32, 32, 3, 1)                          
        self.encoder1_2 = ConvBNPReLU(32, 32, 3, 1)

        self.sample1 = InputInjection(1)   # downsampling the input by a factor of 2 => (H/2, W/2)
        self.sample2 = InputInjection(2)   # downsampling the input by a factor of 4 => (H/4, W/4)
    ...
    def forward(self, input):
        ...
        """
        # stage 1
        output0 = self.encoder1_0(input)
        output0 = self.encoder1_1(output0)
        output0 = self.encoder1_2(output0)
